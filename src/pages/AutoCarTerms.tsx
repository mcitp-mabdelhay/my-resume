...
import { termsOfUseTranslations } from '../data/auto-car/termsOfUse';
import type { Language } from '../components/auto-car/translations';

export function AutoCarTerms() {
  ...
  const t = termsOfUseTranslations[lang];
  const isRtl = lang === 'ar';
  ...
  return (
    ...
    >
      <Helmet>
        <title>
          {isRtl
            ? 'شروط الاستخدام واتفاقية المستخدم | AutoTracker'
            : 'Terms of Use & User Agreement | AutoTracker'}
        </title>
        <meta
          name="description"
          content={
            isRtl
              ? 'شروط الاستخدام الرسمية لتطبيق AutoTracker، اتفاقية ترخيص المستخدم، ملكية البيانات، وإخلاء المسؤولية الميكانيكية.'
              : 'Official Terms of Use for AutoTracker mobile application, covering data ownership, service guidelines, and mechanical disclaimers.'
          }
        />
        <meta property="og:title" content="AutoTracker Terms of Use" />
        <meta
          property="og:description"
          content="AutoTracker terms of service, data rights, and usage guidelines."
        />
        <link rel="canonical" href="https://mabdelhay.com/auto-car/terms" />
      </Helmet>
          ...
          <div className="max-w-4xl mx-auto text-center space-y-4">
            ...
            <div className="flex flex-wrap items-center justify-center gap-x-6 gap-y-2 pt-4 text-xs text-slate-400">
              <span className="flex items-center gap-1.5">
                <strong className="text-slate-200">{isRtl ? 'التطبيق:' : 'App:'}</strong> {t.appName}
              </span>
              <span className="hidden sm:inline">•</span>
              <span className="flex items-center gap-1.5">
                <strong className="text-slate-200">{isRtl ? 'المطور:' : 'Developer:'}</strong> {t.developerName}
              </span>
              <span className="hidden sm:inline">•</span>
              <span className="flex items-center gap-1.5">
                <strong className="text-slate-200">{isRtl ? 'تاريخ التحديث:' : 'Last Updated:'}</strong> {t.lastUpdated}
              </span>
            </div>
          </div>
        </div>
            ...
            <div className="sticky top-28 space-y-6">
              ...
              <div className="p-6 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm space-y-3">
                ...
                <p className="text-xs text-slate-600 dark:text-slate-400 leading-relaxed">
                  {isRtl
                    ? 'تعرف على كيفية حماية بياناتك وتفاصيل توافق تطبيق AutoTracker مع شروط الاستخدام المحدود لـ Google API.'
                    : 'Learn how your vehicle data is protected and read our Google API Limited Use compliance disclosures.'}
                </p>
                <Link
                  to="/auto-car/privacy"
          ...
          <article className="lg:col-span-8 order-1 lg:order-2 space-y-10">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className="p-5 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm space-y-2">
                ...
                <h4 className="font-bold text-sm text-slate-900 dark:text-white">
                  {isRtl ? 'معمارية مباشرة' : 'Direct-to-Cloud'}
                </h4>
                <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
                  {isRtl
                    ? 'بياناتك في حساب Google الشخصي الخاص بك فقط، دون خوادم طرف ثالث وسيطة.'
                    : 'Your logs reside only in your personal Google account with zero intermediary servers.'}
                </p>
              </div>
              ...
              <div className="p-5 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm space-y-2">
                ...
                <h4 className="font-bold text-sm text-slate-900 dark:text-white">
                  {isRtl ? 'إخلاء ميكانيكي' : 'Informational Only'}
                </h4>
                <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
                  {isRtl
                    ? 'تقديرات وتذكيرات تنظيمية لا تغني عن الفحص الميكانيكي المعتمد للسيارة.'
                    : 'Estimates and logs for convenience; not a substitute for certified mechanic inspection.'}
                </p>
              </div>
              ...
              <div className="p-5 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm space-y-2">
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
                  {isRtl
                    ? 'تستطيع تصدير أو تعديل أو حذف بياناتك وإلغاء صلاحية الوصول في أي وقت.'
                    : 'Export, edit, or delete your files, or revoke OAuth access anytime with no lock-in.'}
                </p>
              </div>
            </div>
            ...
            <div className="pt-10 border-t border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-4">
              <div className="flex flex-wrap items-center gap-3">
                ...
                <Link
                  to="/auto-car/privacy"
                  className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-6 py-3 rounded-xl font-bold text-sm bg-slate-200 dark:bg-slate-800 hover:bg-slate-300 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-200 transition-all border border-slate-300 dark:border-slate-700"
                >
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400 text-center sm:text-right">
                {isRtl
                  ? 'تمت صياغة هذه الشروط لتوضيح حقوقك والتزاماتك بشفافية تامة.'
                  : 'Crafted with complete transparency to protect your rights and vehicle records.'}
              </p>
            </div>
      ...
      <footer className="bg-slate-900 text-slate-400 border-t border-slate-800 py-8">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs">
          <div className="flex items-center gap-2">
            <Scale className="w-4 h-4 text-emerald-400" />
            <span>AutoTracker • {t.title}</span>
          </div>
          <div className="flex items-center gap-4">
            <Link to="/auto-car/privacy" className="hover:text-emerald-400 transition-colors">
              {t.privacyLinkText}
            </Link>
            <span>•</span>
            <p>© {new Date().getFullYear()} {t.developerName}. All rights reserved.</p>
          </div>
        </div>
      </footer>
    </div>
